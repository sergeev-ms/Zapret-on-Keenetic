...
### P.S. Это не VPN и не прокси, это минисофт для обмана (подмены) пакетов tcp и udp протокола, повышается анонимность, а также не ломает то, что и так хорошо работало.
...
### (опционально) Если необходимо направить трафик не на всю сеть, а например только гостевую сеть, vlan, опеределенный порт или l2tp, уберите решетку в #IFACE_LAN=eth0 и укажите 1 или несколько интерфейсов (узнать какой интерфейс за что отвечает, можно все той же командой ifconfig)
```bash
IFACE_LAN=br1
```
Или br1 - это бридж основной сети, br2 - vlan гостевой сети, ezcfg0 - частный VPN сервер IKEv2/IPsec и т.д.
```bash
IFACE_LAN="br1 br2 ezcfg0"
```
